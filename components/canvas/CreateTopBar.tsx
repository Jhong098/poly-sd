...
import { Play, Pause, Square, LayoutGrid, ChevronDown, ChevronLeft, Upload, X, Plus } from 'lucide-react'
  ...
  return (
    <header className="h-11 flex-shrink-0 flex items-center gap-3 px-4 bg-raised border-b border-edge relative">
      ...
      <div className="ml-auto flex items-center gap-3">
        ...
        {/* Publish Challenge — always rendered */}
        <button
          onClick={openPublishWizard}
          disabled={status !== 'complete'}
          title={status !== 'complete' ? 'Run the simulation first' : 'Publish this challenge'}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-cyan hover:bg-cyan/90 disabled:opacity-40 disabled:cursor-not-allowed text-base text-[11px] font-bold uppercase tracking-wider transition-colors"
        >
          <Upload size={12} /> Publish Challenge
        </button>
      </div>
