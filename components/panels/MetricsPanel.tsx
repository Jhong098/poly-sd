...
export function MetricsPanel() {
  ...
  return (
    <div data-testid="metrics-panel" className="flex-shrink-0 border-t border-edge bg-raised">
      {/* Mobile summary row — hidden on desktop */}
      <div className="md:hidden flex items-center gap-4 px-4 h-12">
        <div className="flex items-center gap-1.5">
          <div className="w-1.5 h-1.5 rounded-full" style={{ background: statusDotColor }} />
          <span className="text-[10px] text-ink-3">{simTimeSec}s</span>
        </div>
        <span className="text-[11px] font-bold" style={{ color: snap ? latencyColor : 'var(--color-ink-3)' }}>
          {snap ? `${Math.round(p99)}ms` : '—'}
        </span>
        <span className="text-[11px] font-bold text-cyan">
          {snap ? `${Math.round(rps).toLocaleString()} RPS` : '—'}
        </span>
        <span className="text-[11px] font-bold" style={{ color: snap ? errorColor : 'var(--color-ink-3)' }}>
          {snap ? `${(err * 100).toFixed(2)}%` : '—'}
        </span>
        <span className="text-[11px] font-bold text-ink-2 ml-auto">
          {snap ? `$${cost.toFixed(3)}/hr` : ''}
        </span>
        <button
          onClick={() => setMobileExpanded((v) => !v)}
          className="text-ink-3 hover:text-ink-2 text-[10px] font-bold uppercase tracking-wider transition-colors"
          aria-label={mobileExpanded ? 'Collapse metrics' : 'Expand metrics'}
        >
          {mobileExpanded ? '▼' : '▲'}
        </button>
      </div>

      {/* Full panel — always visible on desktop, toggle on mobile */}
      <div className={`h-[88px] px-6 items-center gap-8 ${mobileExpanded ? 'flex' : 'hidden md:flex'}`}>
        {/* Status dot + sim time */}
